...
\title{\textbf{Visual Trading System}\\[0.4em]
  ...
  {\normalsize Final Practicum Report --- Georgia Tech OMSA}\\[0.2em]
  {\small CSE 6748 --- Spring 2026}}
\author{Bob Katz}
\date{April 2026}
...
OMSA Practicum tournament in partnership with the Trilemma Foundation.  The
midterm phase implemented a Gramian Angular Field (GAF) convolutional neural
network classifier that failed to outperform uniform dollar-cost averaging
($41.43\%$ vs.\ $41.94\%$ recency-weighted percentile).  Post-midterm work
pivoted to OLS-based signal engineering, producing the champion strategy: a
252-day rolling z-score of a two-horizon OLS trend-strength signal, winsorized at
$\pm2\sigma$ and mapped via a contrarian weight tilt.  Evaluated across 3,076
rolling 365-day windows (2016--2025), the champion achieves $\mathbf{44.95\%}$
recency-weighted (RW) percentile and $66.94\%$ window win rate---a persistent
$+3.01$ percentage-point improvement over the neutral DCA baseline.  Two
high-potential variants (volatility-normalized amplitude scaling; weekly OLS
resampling) were tested against pre-specified acceptance criteria and rejected as
documented null results, narrowing the timing edge to the z-score normalization
structure specifically.  The strategy is deterministic, look-ahead-free, and
torch-independent via an automatic OLS fallback.

\medskip\noindent\textit{Note: This project uses exclusively public data
(BTC-USD daily prices via Coinbase/yfinance).  No proprietary or confidential
information requires redaction.}
\end{abstract}
...
This project draws on three streams of prior work: classical technical analysis,
image-based financial time-series classification, and time-series-to-image
encoding methods.

\subsection{Classical Technical Analysis}

The visual interpretation of price charts as a basis for trading decisions dates
to the foundational work of Edwards, Magee, and Bassetti, whose
\emph{Technical Analysis of Stock Trends} (first published 1948, now in its tenth
edition) codified the visual pattern framework used by practitioners for over
seven decades~\cite{edwards2018}.  Their central thesis---that recurring chart
formations (head-and-shoulders, double tops, triangles, etc.) encode actionable
information about future price movements---provided the conceptual motivation for
this project's midterm hypothesis: if human traders extract value from visual
patterns, a convolutional neural network trained on chart images might do so more
consistently.

\subsection{Image-Based Financial Classification}

Cohen, Balch, and Veloso~\cite{cohen2020} at J.P.~Morgan AI Research formalized
the connection between visual trading and supervised classification.  Visiting
institutional trading floors, they observed that professional traders execute
orders while \emph{observing images} of financial time series rather than
processing raw numerical data.  They encoded candlestick (Box and Whisker) charts
as images, labeled them using three algebraically-defined binary trade strategies,
and trained over a dozen classification models.  Their key finding---that
image-based classifiers efficiently recover the label-generating rules when data
is visually represented---motivated our use of CNN image classification on
Bitcoin candlestick patterns.

In a follow-up study, Sood, Zeng, Cohen, Balch, and Veloso~\cite{sood2021}
extended this line of work from classification to forecasting, proposing a visual
time-series forecasting framework that captures input data as images and trains a
model to produce subsequent images (predicting distributions rather than point
values).  Their finding that visual forecasting is effective for cyclic data ``but
somewhat less for irregular data such as stock price'' foreshadowed one of this
project's key lessons: high classification accuracy on chart patterns does not
automatically translate to a trading edge when the evaluation metric rewards
intra-window timing rather than directional prediction.

\subsection{Gramian Angular Field Encoding}

The Gramian Angular Field (GAF) encoding used in this project's midterm phase was
introduced by Wang and Oates~\cite{wang2015}, who proposed encoding time series as
images via polar coordinate transformations for use with tiled convolutional
neural networks.  The GAF transform preserves temporal ordering in a 2D structure,
enabling standard image classification architectures to process time-series data
without modification.  Our implementation used the \texttt{pyts} Python
library's \texttt{GramianAngularField} class, which provides both the Gramian
Angular Summation Field (GASF) and Gramian Angular Difference Field (GADF)
variants described in the original paper.

\subsection{Gap Addressed by This Work}

While the J.P.~Morgan papers demonstrated that image-based classification can
recover known trading rules from visual patterns, they did not evaluate
performance under an accumulation-style tournament metric (sats-per-dollar within
rolling windows).  This project extends their line of inquiry by testing the
CNN/GAF pipeline against a realistic evaluation harness and documenting the
resulting failure mode---that classification accuracy on candlestick patterns does
not imply exploitable timing advantage under an SPD metric.  The post-midterm
pivot to OLS signal engineering represents a complementary approach: rather than
classifying visual patterns, we directly estimate regime-relative trend strength
and map it to allocation weights.
...
patterns encode actionable price momentum---a hypothesis grounded in the
classical technical analysis literature~\cite{edwards2018} and supported by
recent work from J.P.~Morgan AI Research demonstrating that CNNs can
efficiently recover trade-relevant label-generating rules from financial chart
images~\cite{cohen2020,sood2021}.  If a convolutional neural network
...
Summation Field (GASF)~\cite{wang2015} is defined as:
...
\begin{enumerate}
  ...
  \item \textbf{Classification accuracy $\neq$ trading edge.}  Even accurate
    ...
    peaks---even if the 30-day return is positive---loses on SPD.  This finding
    is consistent with the caveats raised by Sood et~al.~\cite{sood2021}
    regarding the limitations of visual methods on irregular financial data.
...
This project produced several insights that extend beyond the specific
Bitcoin accumulation problem:

\begin{enumerate}
  \item \textbf{Classification accuracy does not imply a trading edge.}
    The CNN/GAF pipeline was technically sound---deterministic, properly
    cross-validated, and free of look-ahead bias---yet it underperformed a
    trivial baseline.  The failure was not in the model but in the assumption
    that pattern classification transfers to accumulation timing.  This
    distinction between proxy metrics (F1 score) and task-relevant metrics
    (sats-per-dollar) is a recurring pitfall in applied ML that
    Cohen~et~al.~\cite{cohen2020} alluded to but did not test under
    tournament conditions.

  \item \textbf{Simple, interpretable models can outperform complex ones.}
    A single-line OLS z-score signal ($+3.01$\,pp) outperformed a multi-layer
    CNN ($-0.51$\,pp) by $+3.52$\,pp.  The advantage of simplicity is not
    merely aesthetic: the OLS signal's transparency enabled the ablation
    studies that confirmed \emph{why} it works (regime-relative normalization)
    and ruled out alternative explanations.

  \item \textbf{Null results strengthen methodology.}
    The two rejected variants (vol-amplitude scaling, weekly resampling) are
    among the project's most valuable findings.  They narrow the explanation
    of the $+3.01$\,pp edge to a specific mechanism (252-day z-score
    normalization) and increase confidence that the result is not an artifact
    of design flexibility or post-hoc parameter tuning.

  \item \textbf{Pre-specified acceptance criteria prevent p-hacking.}
    Defining the adopt/reject decision rule \emph{before} running each
    experiment---and documenting the criteria alongside the results---imposed
    a discipline that reduced the temptation to rationalize marginal
    improvements.

  \item \textbf{Reproducibility is a deliverable, not an afterthought.}
    The 62-test suite, torch fallback, smoke test ladder, and bounded-actor
    contract were built alongside the signal, not added retroactively.  This
    investment paid off during the sponsor smoke test, which caught a
    Polymarket fixture issue that would have caused a grader-facing failure
    in a fresh clone.
\end{enumerate}
...
\emph{Technical Analysis of Stock Trends}, 10th~ed.
CRC Press, 2018.

\bibitem{cohen2020}
N.~Cohen, T.~H. Balch, and M.~M.~A. Veloso,
``Trading via Image Classification,''
in \emph{Proc.\ ACM International Conference on AI in Finance (ICAIF~'20)},
New York, NY, USA, Oct.\ 2020, pp.\ 1--6.
\newblock \textsc{doi}: \href{https://doi.org/10.1145/3383455.3422544}{10.1145/3383455.3422544}.

\bibitem{sood2021}
S.~Sood, Z.~Zeng, N.~Cohen, T.~Balch, and M.~Veloso,
``Visual Time Series Forecasting: An Image-driven Approach,''
in \emph{Proc.\ 2nd ACM International Conference on AI in Finance (ICAIF~'21)},
Virtual Event, Nov.\ 2021, pp.\ 1--9.
\newblock \textsc{doi}: \href{https://doi.org/10.1145/3490354.3494387}{10.1145/3490354.3494387}.
arXiv:2011.09052.

\bibitem{wang2015}
Z.~Wang and T.~Oates,
``Encoding Time Series as Images for Visual Inspection and Classification
Using Tiled Convolutional Neural Networks,''
in \emph{Workshops at the Twenty-Ninth AAAI Conference on Artificial
Intelligence}, 2015.
